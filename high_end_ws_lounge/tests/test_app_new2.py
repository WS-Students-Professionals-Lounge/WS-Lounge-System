...
def test_reservation_is_submitted_for_admin_review(app, client):
    with app.app_context():
        ...
        if not payment_info:
            payment_info = PaymentInfo(
                ...
                instructions="Pay and upload receipt",
            )
        ...
        assert client.post(
            "/auth/login",
            data={"email": member_email, "password": "memberpass"},
        ).status_code == 302
        ...
        assert b"Awaiting admin approval" in response.data
